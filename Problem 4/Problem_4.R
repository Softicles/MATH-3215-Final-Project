...
# Function for returning the columns of matrix A used to approximate alphas to
# get the least squares errors
vector_x_power <- function(d) {
  vector <- x^d
  return(vector)
}

# Function for solving alphas
solve_for_alphas <- function(d) {
  A = create_matrix(d)
...
create_matrix <- function(d) {
  M = matrix(nrow = n, ncol = d + 1)
  i = 0
  while(i < d + 1) {
    M[, i + 1] = vector_x_power(i)
    i = i + 1
  }
  return(M)
}

# Solve for alphas when d = 5
solve_for_alphas(5)

# Solve for alphas when d = 6 -> Got the error "System is computationally
# singular"
solve_for_alphas(6)

# Conclude that d = 4 because the leading coefficient when d = 5 is already very
# close to 0